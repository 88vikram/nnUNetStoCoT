...
class nnUNetLoggerStoCoT(object):
    """
    This class is really trivial. Don't expect cool functionality here. This is my makeshift solution to problems
    arising from out-of-sync epoch numbers and numbers of logged loss values. It also simplifies the trainer class a
    little
    ...
    def __init__(self, verbose: bool = False):
        self.my_fantastic_logging = {
            'mean_fg_dice1': list(),
            'ema_fg_dice1': list(),
            'dice_per_class_or_region1': list(),
            'mean_fg_dice2': list(),
            'ema_fg_dice2': list(),
            'dice_per_class_or_region2': list(),
            'train_losses1': list(),
            'train_losses2': list(),
            'val_losses1': list(),
            'val_losses2': list(),
    ...
    def log(self, key, value, epoch: int):
        ...
        # handle the ema_fg_dice special case! It is automatically logged when we add a new mean_fg_dice
        if key == 'mean_fg_dice1':
            new_ema_pseudo_dice = self.my_fantastic_logging['ema_fg_dice1'][epoch - 1] * 0.9 + 0.1 * value \
                if len(self.my_fantastic_logging['ema_fg_dice1']) > 0 else value
            self.log('ema_fg_dice1', new_ema_pseudo_dice, epoch)

        if key == 'mean_fg_dice2':
            new_ema_pseudo_dice = self.my_fantastic_logging['ema_fg_dice2'][epoch - 1] * 0.9 + 0.1 * value \
                if len(self.my_fantastic_logging['ema_fg_dice2']) > 0 else value
            self.log('ema_fg_dice2', new_ema_pseudo_dice, epoch)
    ...
    def plot_progress_png(self, output_folder):
        ...
        ax2 = ax.twinx()
        x_values = list(range(epoch + 1))
        ax.plot(x_values, self.my_fantastic_logging['train_losses1'][:epoch + 1], color='b', ls='-', label="loss_tr", linewidth=4)
        ax.plot(x_values, self.my_fantastic_logging['val_losses1'][:epoch + 1], color='r', ls='-', label="loss_val", linewidth=4)
        ax2.plot(x_values, self.my_fantastic_logging['mean_fg_dice1'][:epoch + 1], color='g', ls='dotted', label="pseudo dice",
                 linewidth=3)
        ax2.plot(x_values, self.my_fantastic_logging['ema_fg_dice1'][:epoch + 1], color='g', ls='-', label="pseudo dice (mov. avg.)",
                 linewidth=4)
        ax.set_xlabel("epoch")
        ax.set_ylabel("loss")
        ax2.set_ylabel("pseudo dice")
        ...
        ax2 = ax.twinx()
        x_values = list(range(epoch + 1))
        ax.plot(x_values, self.my_fantastic_logging['train_losses2'][:epoch + 1], color='b', ls='-', label="loss_tr", linewidth=4)
        ax.plot(x_values, self.my_fantastic_logging['val_losses2'][:epoch + 1], color='r', ls='-', label="loss_val", linewidth=4)
        ax2.plot(x_values, self.my_fantastic_logging['mean_fg_dice2'][:epoch + 1], color='g', ls='dotted', label="pseudo dice",
                 linewidth=3)
        ax2.plot(x_values, self.my_fantastic_logging['ema_fg_dice2'][:epoch + 1], color='g', ls='-', label="pseudo dice (mov. avg.)",
                 linewidth=4)
        ax.set_xlabel("epoch")
        ax.set_ylabel("loss")
        ax2.set_ylabel("pseudo dice")
