...
class nnUNetTrainerStoCoT(object):
    ...
    def initialize(self):
        if not self.was_initialized:
            ...
            if self.is_ddp:
                ...
                self.network1 = DDP(self.network1, device_ids=[self.local_rank])
    ...
    def plot_network_architecture(self):
        ...
        if self.local_rank == 0:
            try:
                ...
                g = hl.build_graph(self.network1,
    ...
    def do_split(self):
        ...
        if self.fold == "all":
            ...
            if not isfile(splits_file):
                ...
                splits = generate_crossval_split(all_keys_sorted, seed=12345, n_splits=6)
    ...
    def set_deep_supervision_enabled(self, enabled: bool):
        ...
        """
        if self.is_ddp:
            mod = self.network1.module
            mod = self.network2.module
        else:
            mod = self.network1
            mod = self.network2
    ...
    def save_checkpoint(self, filename: str) -> None:
        if self.local_rank == 0:
            if not self.disable_checkpointing:
                if self.is_ddp:
                    mod1 = self.network1.module
                    mod2 = self.network2.module
                else:
                    mod1 = self.network1
                    mod2 = self.network2

                if isinstance(mod1, OptimizedModule):
                    mod1 = mod1._orig_mod
                if isinstance(mod2, OptimizedModule):
                    mod2 = mod2._orig_mod
    ...
    def load_checkpoint(self, filename_or_checkpoint: Union[dict, str]) -> None:
        ##TODO: network2 not included in loading yet
        if not self.was_initialized:
            self.initialize()
        ...
        if self.is_ddp:
            if isinstance(self.network1.module, OptimizedModule):
                self.network1.module._orig_mod.load_state_dict(new_state_dict)
            else:
                self.network1.module.load_state_dict(new_state_dict)

            if isinstance(self.network2.module, OptimizedModule):
                self.network2.module._orig_mod.load_state_dict(new_state_dict)
            else:
                self.network2.module.load_state_dict(new_state_dict)
        else:
            if isinstance(self.network1, OptimizedModule):
                self.network1._orig_mod.load_state_dict(new_state_dict)
            else:
                self.network1.load_state_dict(new_state_dict)

            if isinstance(self.network2, OptimizedModule):
                self.network2._orig_mod.load_state_dict(new_state_dict)
            else:
    ...
    def perform_actual_validation(self, save_probabilities: bool = False):
        ...
        predictor1 = nnUNetPredictor(tile_step_size=0.5, use_gaussian=True, use_mirroring=True,
                                    perform_everything_on_device=True, device=self.device, verbose=False,
                                    verbose_preprocessing=False, allow_tqdm=False)
        predictor2 = nnUNetPredictor(tile_step_size=0.5, use_gaussian=True, use_mirroring=True,
                                    perform_everything_on_device=True, device=self.device, verbose=False,
                                    verbose_preprocessing=False, allow_tqdm=False)
        predictor1.manual_initialization(self.network1, self.plans_manager, self.configuration_manager, None,
                                        self.dataset_json, self.__class__.__name__,
                                        self.inference_allowed_mirroring_axes)
        predictor2.manual_initialization(self.network2, self.plans_manager, self.configuration_manager, None,
                                        self.dataset_json, self.__class__.__name__,
                                        self.inference_allowed_mirroring_axes)
        ...
        with multiprocessing.get_context("spawn").Pool(default_num_processes) as segmentation_export_pool:
            ...
            for i, k in enumerate(dataset_val.keys()):
                ...
                prediction1 = predictor1.predict_sliding_window_return_logits(data)
                prediction1 = prediction1.cpu()

                prediction2 = predictor2.predict_sliding_window_return_logits(data)
                ...
                results.append(
                    segmentation_export_pool.starmap_async(
                        export_prediction_from_logits, (
                            (prediction1, properties, self.configuration_manager, self.plans_manager,
                             self.dataset_json, output_filename_truncated+'_network1', save_probabilities),
                        )
                    )
                )
                results.append(
                    segmentation_export_pool.starmap_async(
                        export_prediction_from_logits, (
                            (prediction2, properties, self.configuration_manager, self.plans_manager,
                             self.dataset_json, output_filename_truncated+'_network2', save_probabilities),
                        )
                    )
                )
                # for debug purposes
                ...
                if next_stages is not None:
                    for n in next_stages:
                        ...
                        output_file1 = join(output_folder, k + '_network1.npz')
                        output_file2 = join(output_folder, k + '_network2.npz')
                        print(output_file1,output_file2)
                        # resample_and_save(prediction, target_shape, output_file, self.plans_manager, self.configuration_manager, properties,
                        #                   self.dataset_json)
                        results.append(segmentation_export_pool.starmap_async(
                            resample_and_save, (
                                (prediction1, target_shape, output_file1, self.plans_manager,
                                 self.configuration_manager,
                                 properties,
                                 self.dataset_json),
                            )
                        ...
                        results.append(segmentation_export_pool.starmap_async(
                            resample_and_save, (
                                (prediction2, target_shape, output_file2, self.plans_manager,
                                 self.configuration_manager,
                                 properties,
                                 self.dataset_json),
                            )
