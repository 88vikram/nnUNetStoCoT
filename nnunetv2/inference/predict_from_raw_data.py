...
class nnUNetPredictor(object):
    ...
    def initialize_from_trained_stocotmodel_folder(self, model_training_output_dir: str,
                                             use_folds: Union[Tuple[Union[int, str]], None],
                                             checkpoint_name: str = 'checkpoint_final.pth',
                                             model_id:int = 1):
        """
        ...
        for i, f in enumerate(use_folds):
            ...
            if model_id==1:
                ...
                parameters.append(checkpoint['network2_weights'])
    ...
    def _internal_maybe_mirror_and_predict(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if mirror_axes is not None:
            ...
            for axes in axes_combinations:
                prediction_this = self.network(torch.flip(x, axes))
                if type(prediction_this) is list:
                    prediction=prediction+torch.flip(prediction_this[0],axes)
                else:
                    prediction=prediction+torch.flip(prediction_this,axes)
